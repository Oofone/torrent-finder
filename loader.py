...
def createReq(url):
    ...
    if(url.split('/')[2] == "torrentproject.se"):
        hdr = {
...
#print url
    ...
    try:
        ...
        print "Sorry, but I'm afraid the site hosting this torrent is down at the moment.. Maybe later bruv"
        sleep(1)
        htmlOperate(createReq(durl),durl, 0, 0)
...
def htmlOperate(req, url, f=0, tr = 0, done = []):
    x = urllib2.urlopen(req)
    soup = bs4.BeautifulSoup(x.read(), 'html.parser')
    ...
    while choice not in ['y', 'Y', 'n', 'N', 'Q', 'q'] :
        ...
        if choice in ['y', 'Y']:
            ...
            os.system('clear')
            print('Refreshing')
            htmlOperate(req, url, f, tr + 1, done)
            return
        else:
            print 'Huh?'
